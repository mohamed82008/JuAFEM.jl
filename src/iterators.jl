...
```
"""
struct CellIterator{dim, N, T, M, TI<:Integer}
    flags::UpdateFlags
    grid::Grid{dim, N, T, M, TI}
    current_cellid::ScalarWrapper{TI}
    nodes::Vector{TI}
    coords::Vector{Vec{dim, T}}
    dh::DofHandler{dim, N, T, M, TI}
    celldofs::Vector{TI}

    function CellIterator{dim, N, T, M, TI}(dh::DofHandler{dim, N, T, M, TI}, flags::UpdateFlags) where {dim, N, T, M, TI}
        cell = ScalarWrapper(TI(0))
        nodes = zeros(TI, N)
        coords = zeros(Vec{dim, T}, N)
        n = ndofs_per_cell(dh)
        celldofs = zeros(TI, n)
        return new{dim, N, T, M, TI}(flags, dh.grid, cell, nodes, coords, dh, celldofs)
    end

    function CellIterator{dim, N, T, M, TI}(grid::Grid{dim, N, T, M, TI}, flags::UpdateFlags) where {dim, N, T, M, TI}
        cell = ScalarWrapper(TI(0))
        nodes = zeros(TI, N)
        coords = zeros(Vec{dim, T}, N)
        return new{dim, N, T, M, TI}(flags, grid, cell, nodes, coords)
    end
end

@inline CellIterator(grid::Grid{dim, N, T, M, TI},     flags::UpdateFlags=UpdateFlags()) where {dim, N, T, M, TI} = CellIterator{dim,N,T,M,TI}(grid, flags)
@inline CellIterator(dh::DofHandler{dim, N, T, M, TI}, flags::UpdateFlags=UpdateFlags()) where {dim, N, T, M, TI} = CellIterator{dim,N,T,M,TI}(dh, flags)

# iterator interface
Base.start(::CellIterator{dim,N,T,M,TI}) where {dim,N,T,M,TI<:Integer} = one(TI)
Base.next(ci::CellIterator{dim,N,T,M,TI}, i::TI) where {dim,N,T,M,TI<:Integer} = (reinit!(ci, i), i+one(TI))
Base.done(ci::CellIterator{dim,N,T,M,TI}, i::TI) where {dim,N,T,M,TI} = i > getncells(ci.grid)
...
@inline onboundary(ci::CellIterator, face::TI) where {TI<:Integer} = ci.grid.boundary_matrix[face, ci.current_cellid[]]
...
function reinit!(ci::CellIterator{dim,N,T,M,TI}, i) where {dim, N, T, M,TI<:Integer}
...
@inline reinit!(fv::FaceValues{dim, T}, ci::CellIterator{dim, N, T}, face::TI) where {dim, N, T, TI<:Integer} = reinit!(fv, ci.coords, face)
